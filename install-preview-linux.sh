...
awk '
  BEGIN { ok=1; previous="" }
  {
    ...
    if ($2!="DEVELOPMENT.txt" && $2!="install-preview-linux.sh" && $2!="install-preview-macos.sh" && $2!="owntransit-preview-0.6.1-darwin-arm64.tar.gz" && $2!="owntransit-preview-0.6.1-linux-amd64.tar.gz" && $2!="owntransit-preview-0.6.1-linux-arm64.tar.gz") ok=0
    previous=$2
  }
  END { exit ok ? 0 : 1 }
...
if test "$role" = relay && test "$action" = install; then
  ...
  if test -n "$setup_url"; then
    ...
    if ( : </dev/tty ) 2>/dev/null; then
      env -i PATH="$PATH" LC_ALL=C /usr/local/bin/owntransit-relay-preview "$@" </dev/tty
    else
      env -i PATH="$PATH" LC_ALL=C /usr/local/bin/owntransit-relay-preview "$@" </dev/null
    fi
