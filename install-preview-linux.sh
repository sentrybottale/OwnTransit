...
umask 077
version=0.1.6
base=https://github.com/sentrybottale/OwnTransit/releases/download/v0.1.6
stage=
...
awk '
  BEGIN { ok=1; previous="" }
  {
    ...
    if ($2!="DEVELOPMENT.txt" && $2!="install-preview-linux.sh" && $2!="owntransit-preview-0.1.6-darwin-arm64.tar.gz" && $2!="owntransit-preview-0.1.6-linux-amd64.tar.gz" && $2!="owntransit-preview-0.1.6-linux-arm64.tar.gz") ok=0
    previous=$2
  }
  END { exit ok ? 0 : 1 }
