#!/bin/sh
# OwnTransit distribution; installs only an unprivileged local Mac client.
main() {
set -eu
...
action=${1:-client}
test "$#" -le 1 || fail 'usage: sh install-macos.sh [client|--uninstall]'
...
alias_path="$bindir/owntransit-client"
owned_alias() {
  test -L "$1" && test "$(stat -f %u "$1")" = "$uid" && test "$(readlink "$1")" = "$target/owntransit-client"
}
check_release() {
  checked=${1:-$target}
  protected "$checked"
  ...
  for member in CAPSULE LICENSE NOTICE SHA256SUMS owntransit-client install-macos.sh; do
    file="$checked/$member"
    test -f "$file" && test ! -L "$file" && test "$(stat -f %l "$file")" = 1 && test "$(stat -f %u "$file")" = "$uid" || fail 'unsafe installed release member'
    case "$member" in owntransit-client|install-macos.sh) mode=100755 ;; *) mode=100644 ;; esac
...
if test "$action" = --uninstall; then
  ...
  rm -- "$alias_path"
  for member in CAPSULE LICENSE NOTICE SHA256SUMS owntransit-client install-macos.sh; do rm -- "$target/$member"; done
  rmdir "$target"
...
printf '%s\n' "$top/" "$top/CAPSULE" "$top/LICENSE" "$top/NOTICE" "$top/SHA256SUMS" "$top/owntransit-client" "$top/install-macos.sh" | sort > "$stage/expected"
...
chmod 0755 "$stage/$top/owntransit-client" "$stage/$top/install-macos.sh"
...
for old_alias in "$bindir/owntransit-preview" "$bindir/owntransit"; do
  if test ! -L "$old_alias" || test "$(stat -f %u "$old_alias")" != "$uid"; then continue; fi
  old_target=$(readlink "$old_alias")
  case "$old_target" in
    "$software/0.2.0/owntransit"|"$software/0.3.0/owntransit"|"$software/0.4.0/owntransit"|"$software/0.5.0/owntransit"|"$software/0.6.0/owntransit"|"$software/0.6.1/owntransit") ;;
    *) continue ;;
  esac
  old_release=${old_target%/owntransit}
  protected "$old_release"
  test -f "$old_target" && test ! -L "$old_target" && test "$(stat -f %u:%p:%l "$old_target")" = "$uid:100755:1" || fail 'previous client executable metadata differs'
  test "$(readlink "$old_alias")" = "$old_target" || fail 'previous client command changed during installation'
  rm -- "$old_alias"
done
