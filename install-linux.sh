#!/bin/sh
# Signed OwnTransit distribution. The legacy 0.1.0 package lane is separate.
main() {
set -eu
...
cleanup() {
  ...
  if test -n "$stage"; then
    case "$stage" in /var/lib/owntransit-download.*) ;; *) exit 1 ;; esac
    suffix=${stage#/var/lib/owntransit-download.}
...
trap 'exit 130' INT
trap 'exit 143' TERM
test "$#" -ge 1 || fail 'usage: sudo sh install-linux.sh client|target|relay [PUBLIC_RELAY_URL] [--instance NAME] [--uninstall]'
role=$1
shift
setup_url=
...
case "$role" in client|target|relay) ;; *) fail 'role must be client, target or relay' ;; esac
...
stage=$(mktemp -d /var/lib/owntransit-download.XXXXXXXX)
...
if test "$role" = relay; then
  ...
  elif test "$instance_set" = yes; then
    printf 'Selected relay instance: %s\n' "$relay_instance"
  else
    printf '%s\n' 'THIS MACHINE: public relay VPS. Setup asks for the public URL and selects its matching local relay.'
  fi
fi
...
awk '
  BEGIN { ok=1; previous="" }
  {
    ...
    if ($2!="DEVELOPMENT.txt" && $2!="install-linux.sh" && $2!="install-macos.sh" && $2!="owntransit-0.7.0-darwin-arm64.tar.gz" && $2!="owntransit-0.7.0-linux-amd64.tar.gz" && $2!="owntransit-0.7.0-linux-arm64.tar.gz") ok=0
    previous=$2
  }
  END { exit ok ? 0 : 1 }
' "$stage/DEVELOPMENT-SHA256SUMS" || fail 'malformed signed development inventory'

top=owntransit-$version-linux-$arch
archive=$top.tar.gz
...
printf '%s\n' "$top/" "$top/CAPSULE" "$top/LICENSE" "$top/NOTICE" "$top/SHA256SUMS" \
  "$top/install-linux.sh" "$top/owntransit-client" "$top/owntransit-target" \
...
chmod 0755 "$stage/$top/install-linux.sh" "$stage/$top/owntransit-client" "$stage/$top/owntransit-target" "$stage/$top/owntransit-relay"
...
if test "$role" = relay && test "$action" = install; then
  ...
  if test -n "$setup_url"; then
    ...
    if ( : </dev/tty ) 2>/dev/null; then
      env -i PATH="$PATH" LC_ALL=C /usr/local/bin/owntransit-relay "$@" </dev/tty
    else
      env -i PATH="$PATH" LC_ALL=C /usr/local/bin/owntransit-relay "$@" </dev/null
    fi
