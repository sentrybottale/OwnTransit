...
umask 077
version=0.6.1
base=https://github.com/sentrybottale/OwnTransit/releases/download/v0.6.1
...
if test "$action" = --uninstall; then
  test -e "$target" || { printf '%s\n' 'OwnTransit 0.6.1 client is not installed here.'; exit 0; }
...
awk 'BEGIN {ok=1;p=""} {if(NF!=2 || length($1)!=64 || $1!~/^[0-9a-f]+$/ || $0!=$1 "  " $2 || seen[$2]++ || (p!="" && p>=$2))ok=0; if($2!="DEVELOPMENT.txt" && $2!="install-preview-linux.sh" && $2!="install-preview-macos.sh" && $2!="owntransit-preview-0.6.1-darwin-arm64.tar.gz" && $2!="owntransit-preview-0.6.1-linux-amd64.tar.gz" && $2!="owntransit-preview-0.6.1-linux-arm64.tar.gz")ok=0;p=$2} END {exit ok?0:1}' "$stage/DEVELOPMENT-SHA256SUMS" || fail 'malformed release inventory'
...
if test -e "$alias_path" || test -L "$alias_path"; then
  if ! owned_alias "$alias_path"; then
    ...
    case "$previous_target" in
      ...
      "$software/0.6.0/owntransit") previous_version=0.6.0 ;;
...
printf 'NEXT — on THIS Mac, without sudo:\n  '; quote "$command_path"; printf ' pair setup\n'
printf '%s\n' 'Then answer its prompts: your relay URL and the private receiver code (otpair2.).'
printf 'Another tunnel:\n  '; quote "$command_path"; printf ' pair setup --tunnel office\n'
printf 'Interrupted or unsure? Run here:\n  '; quote "$command_path"; printf ' pair next\n'
printf '%s\n' 'A new tunnel remains UNDER CONSTRUCTION until the client end-to-end check succeeds.'
printf 'Uninstall (retains pairing):\n  sh '; quote "$target/install-macos.sh"; printf ' --uninstall\n'
}
main "$@"
