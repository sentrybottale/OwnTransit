...
its route-authority keys off the relay host.
...
The relay requires an existing systemd installation. On Debian/Ubuntu, the
script installs Podman through APT if it is missing; this may install or update
required dependencies and their distro-provided helper units. It does not run
a system-wide upgrade, remove packages, or reconfigure an existing Podman
installation. Other hosts must provide Podman at `/usr/bin/podman` themselves.

OwnTransit does not edit Nginx, websites, firewall rules or SSH configuration.
A fresh relay is disabled and publishes only `127.0.0.1:9087` when activated.
Configure public HTTPS forwarding separately under the VPS operator's control;
the installer does not claim port 80/443 or replace an existing web server.
Podman's bridge networking is created when containers are explicitly started,
not by an OwnTransit firewall configuration step.

Start a new login session before using the client. To refresh the current
terminal, run `sudo -iu YOUR_LOGIN` first, then type `owntransit version` at the
new prompt. Fresh relay and connector services stay disabled and stopped until
enrollment; an update preserves existing service state. Installation does not
create SSH keys or enroll any role.

If `curl` is missing on Debian/Ubuntu, install the download prerequisite first:
...
Installing the client does not create `invitation.otinvite`. It must be a real
file generated by the administrator with `owntransit-provision issue-invitation`.
For your own new deployment, follow [First deployment](FIRST_DEPLOYMENT.md): it
covers the relay, connector, route authority, actual invitation creation,
approval, and service activation. If someone else manages OwnTransit for you,
ask them for your invitation and skip the administrator steps.

<details>
<summary>Interrupted package operations and installation trust</summary>
...
For the relay role, use the corresponding installed command under
`roles/relay/current/owntransitctl` with `package-recover --role relay`.
