...
case "$(uname -m)" in
  ...
  *) fail "this installer supports Linux amd64 and arm64 only" ;;
esac
...
case "$role" in
  client)
    artifact_name="owntransit-linux-$platform_arch"
    ...
    ;;
  connector)
    artifact_name="owntransit-connector-linux-$platform_arch"
    ...
    ;;
  relay)
    artifact_name="owntransit-relay-linux-$platform_arch.oci.tar"
    installed_name=
    needs_lifecycle=yes
    ...
    ;;
  provisioner)
    artifact_name="owntransit-provision-linux-$platform_arch"
    ...
    -v expected_arch="$platform_arch" '
      {
        if (NR != 1 || NF != 41 ||
            ...
            $30 != "protocol" || $31 != ":" || $33 != "," ||
            $34 != "goos" || $35 != ":" || $36 != "linux" || $37 != "," ||
            $38 != "goarch" || $39 != ":" || $40 != expected_arch || $41 != "}") {
          invalid = 1
          next
        }
...
if test "$needs_lifecycle" = yes; then
  lifecycle_path="artifacts/owntransitctl-linux-$platform_arch"
...
require_selected_lifecycle_directory() {
  ...
  test "$(stat -c %u "$selected_directory")" -eq 0 ||
    fail "selected lifecycle ancestor is not root-owned: $selected_directory"
  selected_mode=$(stat -c %a "$selected_directory")
...
guard_retained_prerelease_install() {
  selected_current_link="/usr/libexec/owntransit/roles/$role/current"
  test -e "$selected_current_link" || test -L "$selected_current_link" || return 0

  for selected_directory in \
    / /usr /usr/libexec /usr/libexec/owntransit /usr/libexec/owntransit/roles \
    "/usr/libexec/owntransit/roles/$role" "/usr/libexec/owntransit/roles/$role/releases"; do
  ...
  selected_release_directory="/usr/libexec/owntransit/roles/$role/releases/$selected_release"
