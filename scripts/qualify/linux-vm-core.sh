...
# Internal shared implementation. Invoke linux-vm.sh or one fixed-architecture
# wrapper; direct use is unsupported and never changes the host/arch checks.
...
test "${1:-}" = --qualification-architecture && test "$#" -ge 3 || {
  printf '%s\n' 'linux-vm-core: fixed wrapper architecture and phase are required' >&2
  exit 2
}
qualification_arch=$2
shift 2
case "$qualification_arch" in
  amd64|arm64) ;;
  *)
    printf 'linux-vm: unsupported qualification architecture: %s\n' "$qualification_arch" >&2
    exit 2
    ;;
esac

program=linux-$qualification_arch-vm
...
connector_artifact=artifacts/owntransit-connector-linux-$qualification_arch
...
usage() {
  cat <<'EOF'
usage:
  linux-{amd64,arm64}-vm.sh preflight|prepare \
  ...
  linux-{amd64,arm64}-vm.sh verify-after-reboot
...
check_host_boundary() {
  ...
  test "$host_arch" = "$qualification_arch" || fail "qualification entry point requires $qualification_arch but this machine is $host_arch"
...
check_bundle() {
  ...
  connector_digest=$(listed_digest "$connector_artifact")
  lifecycle_digest=$(listed_digest "$lifecycle_artifact")
  relay_digest=$(listed_digest "$relay_artifact")
  client_digest=$(listed_digest "$client_artifact")
  provision_digest=$(listed_digest "$provisioner_artifact")
  ...
  test "$(sha256sum "$bundle/$connector_artifact" | awk '{print $1}')" = "$connector_digest" || fail "connector artifact checksum mismatch"
  test "$(sha256sum "$bundle/$lifecycle_artifact" | awk '{print $1}')" = "$lifecycle_digest" || fail "lifecycle artifact checksum mismatch"
  test "$(sha256sum "$bundle/$provisioner_artifact" | awk '{print $1}')" = "$provision_digest" || fail "provisioner artifact checksum mismatch"
  file -b "$bundle/$connector_artifact" | grep -Fq 'ELF 64-bit LSB' || fail "connector is not an ELF executable"
  case "$qualification_arch" in
    amd64) file -b "$bundle/$connector_artifact" | grep -Fq 'x86-64' || fail "connector is not amd64" ;;
    arm64) file -b "$bundle/$connector_artifact" | grep -Fq 'ARM aarch64' || fail "connector is not arm64" ;;
  esac
  strings "$bundle/$connector_artifact" | grep -Fq 'tcp4 127.0.0.1:22' || fail "connector does not identify the production target"
  if strings "$bundle/$connector_artifact" | grep -Fq '127.0.0.1:2222'; then
    fail "connector contains the POC target"
  fi
  version_json=$("$bundle/$connector_artifact" version)
  ...
  printf '%s\n' "$version_json" | grep -Fq "\"goarch\":\"$qualification_arch\"" || fail "connector version architecture mismatch"
...
if test "$phase" = verify-after-reboot; then
  ...
  recorded_architecture=$(awk -F= '$1 == "architecture" { print $2 }' "$phase_file")
  ...
  test "$recorded_architecture" = "$qualification_arch" || fail "recorded qualification architecture does not match this entry point"
...
if test "$phase" = preflight; then
  printf '{"schema":"owntransit.qualify.linux-%s-preflight.v1","result":"pass","platform":"linux","architecture":"%s","pid1":"systemd","disposable_marker":true,"host_pristine":true,"protected_hardlinks":true,"release_id":"%s","release_sequence":%s,"checksums_sha256":"%s","connector_sha256":"%s","unit_sha256":"%s","network_required":false,"reboot_invoked":false}\n' \
    "$qualification_arch" "$qualification_arch" "$release_id" "$release_sequence" "$checksums_sha256" "$connector_digest" "$unit_digest"
  exit 0
fi
...
provision="$bundle/$provisioner_artifact"
ctl="$bundle/$lifecycle_artifact"
...
"$ctl" bootstrap \
  ...
  --release-id "$release_id" --release-sequence "$release_sequence" --artifact-sha256 "$relay_digest" --os linux --arch "$qualification_arch" \
...
"$ctl" bootstrap \
  ...
  --release-id "$release_id" --release-sequence "$release_sequence" --artifact-sha256 "$client_digest" --os linux --arch "$qualification_arch" \
  --outer-ca "$trust_root/outer-endpoint-ca-cert.pem" --inner-connector-ca "$trust_root/inner-connector-ca-cert.pem" \
  --inner-client-ca "$trust_root/inner-client-capability-ca-cert.pem" --deployment-signer "$trust_root/deployment-signing-public.pem" \
...
/usr/libexec/owntransit/roles/connector/current/owntransitctl bootstrap \
  ...
  --release-id "$release_id" --release-sequence "$release_sequence" --artifact-sha256 "$connector_digest" --os linux --arch "$qualification_arch" \
  --outer-ca "$trust_root/outer-endpoint-ca-cert.pem" --inner-connector-ca "$trust_root/inner-connector-ca-cert.pem" \
  --inner-client-ca "$trust_root/inner-client-capability-ca-cert.pem" --deployment-signer "$trust_root/deployment-signing-public.pem" \
...
phase_file="$qualification_root/phase"
printf '%s\n' \
  "schema=owntransit.qualify.linux-$qualification_arch-phase.v1" \
  "release_id=$release_id" \
  "release_sequence=$release_sequence" \
  "architecture=$qualification_arch" \
...
printf '{"schema":"owntransit.qualify.linux-%s-prepare.v1","result":"awaiting-reboot","platform":"linux","architecture":"%s","release_id":"%s","release_sequence":%s,"checksums_sha256":"%s","connector_sha256":"%s","unit_sha256":"%s","prepared_boot_id":"%s","prepared_unix":%s,"unit_enabled":true,"active_state":"active","sub_state":"running","main_pid":%s,"restart_count":0,"protected_hardlinks":true,"platform_mutation_lock":true,"connector_listener_count":0,"qualification_credentials":"throwaway-local","relay_endpoint":"loopback-refused","network_required":false,"reboot_invoked":false}\n' \
  "$qualification_arch" "$qualification_arch" "$release_id" "$release_sequence" "$checksums_sha256" "$connector_digest" "$unit_digest" "$boot_id" "$prepared_unix" "$main_pid" > "$prepare_evidence"
chmod 0644 "$prepare_evidence"
cat "$prepare_evidence"
printf 'NEXT: reboot this disposable VM, then run scripts/qualify/linux-%s-vm.sh verify-after-reboot\n' "$qualification_arch" >&2
