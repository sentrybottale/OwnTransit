...
require_text packaging/homebrew/build-source-archive.sh 'git -c tar.umask=0022 -C "$source_root" archive'
require_text packaging/homebrew/build-source-archive.sh 'tar -xzpf "$archive" -C "$verify_root"'
require_text packaging/homebrew/build-source-archive.sh 'source archive directory mode is not 0755'
require_text packaging/homebrew/build-source-archive.sh 'source archive changed tracked mode'
...
require_text scripts/qualify/linux-vm.sh 'x86_64|amd64) architecture=amd64'
require_text scripts/qualify/linux-vm.sh 'aarch64|arm64) architecture=arm64'
require_text scripts/qualify/linux-vm.sh 'exec "$script_dir/linux-$architecture-vm.sh" "$@"'
require_text scripts/qualify/linux-amd64-vm.sh 'exec "$script_dir/linux-vm-core.sh" --qualification-architecture amd64 "$@"'
require_text scripts/qualify/linux-arm64-vm.sh 'exec "$script_dir/linux-vm-core.sh" --qualification-architecture arm64 "$@"'
require_text scripts/qualify/linux-vm-core.sh 'fixed wrapper architecture and phase are required'
require_text scripts/qualify/linux-vm-core.sh 'OWNTRANSIT_DISPOSABLE_VM=1'
require_text scripts/qualify/linux-vm-core.sh 'aarch64|arm64) host_arch=arm64'
require_text scripts/qualify/linux-vm-core.sh 'test "$host_arch" = "$qualification_arch"'
require_text scripts/qualify/linux-vm-core.sh 'connector_artifact=artifacts/owntransit-connector-linux-$qualification_arch'
require_text scripts/qualify/linux-vm-core.sh 'recorded_architecture=$(awk'
require_text scripts/qualify/linux-vm-core.sh 'test "$recorded_architecture" = "$qualification_arch"'
require_text scripts/qualify/linux-vm-core.sh 'kernel boot ID did not change'
require_text scripts/qualify/linux-vm-core.sh 'connector process owns a TCP listener'
require_text scripts/qualify/linux-vm-core.sh 'wss://127.0.0.1:65535/connects'
require_text scripts/qualify/linux-vm-core.sh '"cold_boot_verified":true'
require_text scripts/qualify/linux-vm-core.sh '"active_package_recovery":true'
require_text scripts/qualify/linux-vm-core.sh 'package-recover --role connector'
require_text scripts/qualify/linux-vm-core.sh '"protected_hardlinks":true'
require_text scripts/qualify/linux-vm-core.sh 'assert_platform_mutation_lock'
require_text scripts/qualify/linux-vm-core.sh 'platform.v1.lock'
require_text scripts/qualify/linux-vm-core.sh 'record_state in intent restart'
require_text scripts/qualify/linux-vm-core.sh '"platform_mutation_lock":true'
require_text scripts/qualify/linux-vm-core.sh 'installer pre-created bootstrap-owned root'
require_text scripts/qualify/linux-vm-core.sh '--state-root /var/lib/owntransit/connector/private'
require_text scripts/qualify/linux-vm-core.sh '--runtime-root /var/lib/owntransit/connector/runtime'
require_text scripts/qualify/linux-vm-core.sh '--anchor-view-root /var/lib/owntransit/connector/anchor-view'
require_text scripts/qualify/linux-vm-core.sh '--reader-gid "$connector_reader_gid"'
require_text scripts/qualify/linux-vm-core.sh 'connector service owns an entry in its role tree'
require_text scripts/qualify/linux-vm-core.sh 'connector created a file in a published view'
require_text scripts/qualify/linux-vm-core.sh 'connector replaced published material'
require_text scripts/qualify/linux-vm-core.sh 'connector renamed published material'
require_text scripts/qualify/linux-vm-core.sh 'connector unlinked published material'
require_text scripts/qualify/linux-vm-core.sh 'connector chown succeeded on published material'
require_text scripts/qualify/linux-vm-core.sh 'private lifecycle root is not root:root 0700'
require_text scripts/qualify/linux-vm-core.sh 'runtime view root is not root:reader 0750'
require_text scripts/qualify/linux-vm-core.sh 'release_sequence=$(build_input release_sequence)'
require_text scripts/qualify/linux-vm-core.sh '--release-sequence "$release_sequence"'
require_text scripts/qualify/linux-vm-core.sh 'assert_status_release_sequence "$connector_status_json" "$release_sequence"'
require_text scripts/qualify/linux-vm-core.sh 'assert_status_release_sequence "$reboot_status_json" "$recorded_release_sequence"'
require_text scripts/qualify/linux-vm-core.sh '"release_sequence":%s'
if grep -Fq -- '--release-sequence 1' scripts/qualify/linux-vm-core.sh; then
  fail 'Linux qualification must use the authenticated candidate release sequence'
fi
require_text .github/workflows/release-candidate.yml 'TestProvisionerPackageLifecycleAppliesAndRollsBackWithoutRuntimeReader|TestInstalledPackageDirectoriesEnforceRoleProfile'
...
require_text scripts/qualify/README.md '`linux-arm64-vm.sh`'
require_text scripts/qualify/README.md 'one architecture cannot satisfy the other'
...
require_text scripts/qualify/linux-relay-exchange.sh 'aarch64|arm64) qualification_arch=arm64'
require_text scripts/qualify/linux-relay-exchange.sh 'relay_artifact=artifacts/owntransit-relay-linux-$qualification_arch.oci.tar'
require_text scripts/qualify/linux-relay-exchange.sh 'stage_executable "$bundle/$client_artifact"'
require_text scripts/qualify/linux-relay-exchange.sh 'verify_native_build_identity "$client" client'
require_text scripts/qualify/linux-relay-exchange.sh 'verify_native_build_identity "$provisioner" provisioner'
require_text scripts/qualify/linux-relay-exchange.sh "podman image inspect --format '{{.Architecture}}'"
require_text scripts/qualify/linux-relay-exchange.sh 'test "$installed_image_arch" = "$qualification_arch"'
require_text scripts/qualify/linux-relay-exchange.sh '--arch "$qualification_arch"'
require_text scripts/qualify/linux-relay-exchange.sh '"oci_architecture_verified":true'
...
require_text scripts/qualify/linux-relay-exchange.sh 'relay.intent relay.restart'
...
require_text scripts/qualify/macos-client-boundary.sh 'public client launcher is not root:reader setgid 2751'
require_text scripts/qualify/macos-client-boundary.sh 'public and release launchers differ'
require_text scripts/qualify/macos-client-boundary.sh 'public launcher is a hard link to the release launcher'
require_text scripts/qualify/macos-client-boundary.sh 'target user can directly execute the protected release launcher'
require_text scripts/qualify/macos-client-boundary.sh '"$public_launcher" --qualify-reader-gid'
require_text scripts/qualify/macos-client-boundary.sh 'package-mutation.v1.lock'
require_text scripts/qualify/macos-client-boundary.sh 'private launcher stage contains missing or unexpected transaction state'
require_text scripts/qualify/macos-client-boundary.sh 'ordinary target user could not reproduce the Darwin launcher hard-link condition'
require_text scripts/qualify/macos-client-boundary.sh 'retained launcher hard-link alias acquired reader authority'
require_text scripts/qualify/macos-client-boundary.sh 'canonical launcher was denied solely because an alias existed'
...
require_text internal/securefs/readonly.go 'func (root *ReadOnlyRoot) ReadRootSymlink(name string, limit int) (string, error)'
...
if grep -Eq 'runuser -u owntransit-connector -- .*owntransitctl' scripts/qualify/linux-vm-core.sh; then
  fail 'Linux qualification runs root-only lifecycle as the connector service'
fi
if grep -Fq -- '--state-root=/var/lib/owntransit/connector/state' scripts/qualify/linux-vm-core.sh; then
  fail 'Linux qualification uses the retired service-owned state root'
fi

if grep -Eq '^[[:space:]]*((/usr/sbin/|/sbin/)?reboot|systemctl[[:space:]]+reboot)([[:space:]]|$)' scripts/qualify/linux-vm-core.sh; then
