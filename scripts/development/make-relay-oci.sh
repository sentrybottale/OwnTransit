...
usage() {
  cat <<'EOF'
usage: make-relay-oci.sh BINARY OUTPUT ARCH COMMIT SOURCE_DATE_EPOCH PROJECT_LICENSE THIRD_PARTY_NOTICES
...
printf '%s\n' \
  "{\"schemaVersion\":2,\"manifests\":[{\"mediaType\":\"application/vnd.oci.image.manifest.v1+json\",\"digest\":\"sha256:$manifest_digest\",\"size\":$manifest_size,\"platform\":{\"architecture\":\"$architecture\",\"os\":\"linux\"},\"annotations\":{\"org.opencontainers.image.ref.name\":\"owntransit-relay:0.7.0\"}}]}" \
  > "$layout/index.json"
