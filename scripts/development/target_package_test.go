...
func TestTargetPackageTemplateMigrationPreservesConfinement(t *testing.T) {
	...
	current := strings.ReplaceAll(s[start:start+end+1], "$prefix", "/opt/owntransit/1.0.1")
	...
	for _, tc := range []struct {
		...
		{"previous-canonical", strings.ReplaceAll(current, "1.0.1", "0.7.0"), true},
		{"previous-080", strings.ReplaceAll(current, "1.0.1", "0.8.0"), true},
		{"previous-100", strings.ReplaceAll(current, "1.0.1", "1.0.0"), true},
		{"unknown-canonical", strings.ReplaceAll(current, "1.0.1", "0.7.9"), false},
