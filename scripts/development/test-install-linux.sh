...
bundle=/root/owntransit-preview-0.1.8-linux-$arch
...
grep -Fq 'Then answer its prompts, not with another command' "$client_output"
grep -Fq 'VPS registration code starting with otrelay1.' "$client_output"
grep -Fq 'Private receiving-machine code starting with otpair1.' "$client_output"
test -x /opt/owntransit-preview/0.1.8/client/owntransit
test "$(readlink /usr/local/bin/owntransit-preview)" = /opt/owntransit-preview/0.1.8/client/owntransit
...
test -x /opt/owntransit-preview/0.1.8/connector/owntransit-connector
test "$(readlink /usr/local/bin/owntransit-connector-preview)" = /opt/owntransit-preview/0.1.8/connector/owntransit-connector
test -f /etc/systemd/system/owntransit-connector-pair.service
grep -Fqx 'ExecStart=/opt/owntransit-preview/0.1.8/connector/owntransit-connector pair serve --state /var/lib/owntransit-pair' /etc/systemd/system/owntransit-connector-pair.service
...
sed -e 's/0\.1\.8/0.1.2/g' -e 's/^Type=notify$/Type=simple/' \
...
sed 's/0\.1\.8/0.1.3/g' "$unit" > /tmp/preview-013-unit
...
grep -Fqx 'ExecStart=/opt/owntransit-preview/0.1.8/connector/owntransit-connector pair serve --state /var/lib/owntransit-pair' "$unit"
sed 's/0\.1\.8/0.1.6/g' "$unit" > /tmp/preview-016-unit
...
sed 's/0\.1\.8/0.1.7/g' "$unit" > /tmp/preview-017-unit
...
test ! -e /opt/owntransit-preview/0.1.8/relay || fail 'alias conflict partially installed relay role'
...
test -x /opt/owntransit-preview/0.1.8/relay/owntransit-relay
test -f /opt/owntransit-preview/0.1.8/relay/owntransit-relay.oci.tar
test "$(readlink /usr/local/bin/owntransit-relay-preview)" = /opt/owntransit-preview/0.1.8/relay/owntransit-relay
