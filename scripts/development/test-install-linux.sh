...
bundle=/root/owntransit-preview-0.4.0-linux-$arch
...
grep -Fqx 'Next: /usr/local/bin/owntransit pair setup' "$client_output"
grep -Fq 'VPS registration code (otrelay1.)' "$client_output"
grep -Fq 'private SSH-machine code (otpair1.)' "$client_output"
test -x /opt/owntransit-preview/0.4.0/client/owntransit
test "$(readlink /usr/local/bin/owntransit-preview)" = /opt/owntransit-preview/0.4.0/client/owntransit
...
test -x /opt/owntransit-preview/0.4.0/connector/owntransit-connector
test "$(readlink /usr/local/bin/owntransit-connector-preview)" = /opt/owntransit-preview/0.4.0/connector/owntransit-connector
test -f /etc/systemd/system/owntransit-connector-pair.service
grep -Fqx 'ExecStart=/opt/owntransit-preview/0.4.0/connector/owntransit-connector pair serve --state /var/lib/owntransit-pair' /etc/systemd/system/owntransit-connector-pair.service
...
sed -e 's/0\.4\.0/0.1.2/g' -e 's/^Type=notify$/Type=simple/' \
...
sed 's/0\.4\.0/0.1.3/g' "$unit" > /tmp/preview-013-unit
...
grep -Fqx 'ExecStart=/opt/owntransit-preview/0.4.0/connector/owntransit-connector pair serve --state /var/lib/owntransit-pair' "$unit"
sed 's/0\.4\.0/0.1.6/g' "$unit" > /tmp/preview-016-unit
...
sed 's/0\.4\.0/0.1.7/g' "$unit" > /tmp/preview-017-unit
...
sed 's/0\.4\.0/0.2.0/g' "$unit" > /tmp/preview-020-unit
...
sed 's/0\.4\.0/0.3.0/g' "$unit" > /tmp/preview-030-unit
...
test ! -e /opt/owntransit-preview/0.4.0/relay || fail 'alias conflict partially installed relay role'
...
grep -Fqx 'Next: sudo /usr/local/bin/owntransit-relay setup --instance default' "$relay_output"
if grep -Fq 'podman run' "$relay_output"; then fail 'installer exposed manual container setup'; fi
test -x /opt/owntransit-preview/0.4.0/relay/owntransit-relay
test -f /opt/owntransit-preview/0.4.0/relay/owntransit-relay.oci.tar
test "$(readlink /usr/local/bin/owntransit-relay-preview)" = /opt/owntransit-preview/0.4.0/relay/owntransit-relay
...
grep -Fqx 'Next: sudo /usr/local/bin/owntransit-relay setup --instance alpha' /tmp/relay-alpha-install.out
...
test -x /usr/local/bin/owntransit-relay-preview && test -f /opt/owntransit-preview/0.4.0/relay/owntransit-relay.oci.tar || fail 'scoped removal removed shared software'
...
install -m 0644 /opt/owntransit-preview/0.4.0/connector/service.template "$unit"
...
test "$(cat /usr/local/bin/owntransit)" = unmanaged-command
test ! -e /opt/owntransit-preview/0.4.0/client
"$bundle/install-linux.sh" --bundle "$bundle" --role client --action uninstall
"$bundle/install-linux.sh" --bundle "$bundle" --role client
