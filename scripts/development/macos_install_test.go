...
func TestMacInstallerIsolatedLifecycle(t *testing.T) {
	...
	top := "owntransit-1.0.0-darwin-arm64"
	...
	members := []string{"DEVELOPMENT.txt", "install-linux.sh", "install-macos.sh", top + ".tar.gz", "owntransit-1.0.0-linux-amd64.tar.gz", "owntransit-1.0.0-linux-arm64.tar.gz"}
	...
	for _, priorVersion := range []string{"0.7.0", "0.8.0"} {
		...
		for name, data := range files {
			...
			write(filepath.Join(priorCanonical, name), bytes.ReplaceAll(data, []byte("1.0.0"), []byte(priorVersion)), mode)
		}
		...
		run(true, "client")
		if got, _ := os.Readlink(alias); got != filepath.Join(software, "1.0.0", "owntransit-client") {
