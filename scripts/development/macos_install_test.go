...
func TestMacInstallerIsolatedLifecycle(t *testing.T) {
	...
	top := "owntransit-preview-0.5.0-darwin-arm64"
	...
	members := []string{"DEVELOPMENT.txt", "install-preview-linux.sh", "install-preview-macos.sh", top + ".tar.gz", "owntransit-preview-0.5.0-linux-amd64.tar.gz", "owntransit-preview-0.5.0-linux-arm64.tar.gz"}
	...
	for _, previousVersion := range []string{"0.2.0", "0.3.0", "0.4.0"} {
		previous := filepath.Join(software, previousVersion)
		if err := os.Rename(filepath.Join(software, "0.5.0"), previous); err != nil {
			t.Fatal(err)
		}
		...
		for _, command := range []string{alias, filepath.Join(home, ".local/bin/owntransit")} {
			if to, err := os.Readlink(command); err != nil || to != filepath.Join(software, "0.5.0/owntransit") {
