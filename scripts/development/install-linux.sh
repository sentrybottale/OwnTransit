...
legacy_alias_owned() {
  ...
  case "$old_target" in
    /opt/owntransit/0.7.0/"$role/$binary") ;;
...
if test -e "$alias_path" || test -L "$alias_path"; then
  ...
  case "$previous_alias" in
    "$alias_target") ;;
    /opt/owntransit-preview/*|/opt/owntransit/0.7.0/*)
...
if test "$role" = target; then
  ...
  cat > "$unit_stage" <<EOF
[Unit]
Description=OwnTransit 0.8.0 Target
...
EOF
  ...
  printf 'OwnTransit 0.8.0 Target installed for linux/%s.\n' "$arch"
  ...
  printf 'OwnTransit 0.8.0 Client installed for linux/%s.\n' "$arch"
  printf 'NEXT — on THIS Client computer, without sudo:\n  %s setup\n' "$public_command"
  printf '%s\n' 'Choose New tunnel or Continue tunnel from the menu.'
else
  printf 'OwnTransit 0.8.0 Relay installed for linux/%s.\n' "$arch"
