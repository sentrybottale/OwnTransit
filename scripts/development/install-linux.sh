...
prefix=/opt/owntransit-preview/0.1.7
case "$role" in
...
if test -e "$alias_path" || test -L "$alias_path"; then
  ...
  case "$previous_alias" in
    "$alias_target") ;;
    "/opt/owntransit-preview/0.1.1/$role/$binary"|"/opt/owntransit-preview/0.1.2/$role/$binary"|"/opt/owntransit-preview/0.1.3/$role/$binary"|"/opt/owntransit-preview/0.1.5/$role/$binary"|"/opt/owntransit-preview/0.1.6/$role/$binary")
...
if test "$role" = connector; then
  ...
  cat > "$unit_stage" <<EOF
[Unit]
Description=OwnTransit 0.1.7 preview receiver pairing broker
...
TimeoutStartSec=30s' \
  ...
  printf 'Installed OwnTransit development preview 0.1.7 role %s for linux/%s.\n' "$role" "$arch"
